...
import { OperationPathParameters, OperationQueryParameters } from "./request-parameters";
/**
 * Excludes the <pre>parameters</pre> property if no <pre>path</pre> or <pre>query</pre> exists.
 */
export type FetchOptions<Operation> = Omit<AllFetchOptions<Operation>, (OperationRequestBody<Operation> extends never ? "body" : never) | (OperationPathParameters<Operation> extends never ? OperationQueryParameters<Operation> extends never ? "parameters" : never : never)>;
/**
...
} : (OperationPathParameters<Operation> extends never ? never : {
    path: OperationPathParameters<Operation>;
}) | (OperationQueryParameters<Operation> extends never ? never : {
